...
from facedetectors.yunet import Yunet
from facedetectors.dnn import DNN
from facedetectors.mtcnn import MTCNNDet
from facedetectors.haar import HaarCascades

from mot_trackers.sort_tracker import sorttracker
from mot_trackers.centroid_tracker import centroidtracker

from face_tracker.CVFace_tracker import CVFace_tracker
from face_tracker.CAMSHIFT_tracker import CAMSHIFT_tracker
from face_tracker.KLT_tracker import KLT_tracker

from Image_Transformer.Image_Transformer import Image_Transformer
	...
	def do_text(self, handle, elements):
		...
		for e in elements:
			if isinstance(e, TimingInfo):
				...
				handle.write(' temp={:.0f}C'.format(50))
...
class DetectedFace:
	"""A detected face - simply the information conveyed by the object detector"""
	def __init__(self, bbox, idnum, do_draw):
		self.bbox = bbox
		self.priority = 5
		self.outline = (255, 0, 0)
		self.idnum = idnum
	...
	def do_render(self, render):
		if not self.do_draw:
			pts = list(np.int32(np.array(self.bbox).reshape(-1,2) * render.ratio).reshape(-1))
			topc = []
			topc.append((self.bbox[0]))
			topc.append((self.bbox[1]))
			print("POINTS")
			print(pts)
			render.draw.rectangle(pts, outline=self.outline, fill = (255,255,255))

			text = str(self.idnum)
			render.draw.text(topc, text, align ="center", fill = (255,0,0))
...
class CameraImage:
	...
	def __init__(self, image, nums, model, do_draw):
		self.image = image
		self.priority = 1
		self.raw = True
		(self.faces, self.area) = nums
		if do_draw:
			self.blurred = model.apply_transformation(image, self.area)
		self.do_draw = do_draw
	...
	def do_render(self, render):
		if self.do_draw:
			...
			self.image.paste(self.blurred, mask=mask)
...
class Pipeline:
	...
	def __init__(self, args):
		...
		if 'Yunet' in self.args.facemodel:
			self.face_detector = Yunet("model_files/face_detection_yunet_2022mar.onnx", (200,200), score_threshold = self.args.facescore)
		elif 'MTCNN' in self.args.facemodel:
			self.face_detector = MTCNNDet((200,200), score_threshold = self.args.facescore)
		elif 'Haar' in self.args.facemodel:
			self.face_detector = HaarCascades("model_files/haarcascade_frontalface2.xml", "model_files/haarcascade_profileface.xml", (200,200), score_threshold = self.args.facescore)
		elif 'DNN' in self.args.facemodel:
			self.face_detector = DNN("model_files/res10_300x300_ssd_iter_140000.caffemodel", (300,300), score_threshold = self.args.facescore)
		else:
			print('{} is invalid face detection model'.format(self.args.model))
			sys.exit(1)

		self.dfdata = []
		self.header = ['framenum', 'x', 'y', 'w', 'h', 'id']
		self.image_t = Image_Transformer(self.args.imgtmodel)
		...
		if 'KCF' in self.args.trackermodel:
			self.faceTracker = CVFace_tracker("KCF", 100)
		elif 'KLT' in self.args.trackermodel:
			self.faceTracker = KLT_tracker("KLT", 100)
		elif 'CAMSHIFT' in self.args.trackermodel:
			self.faceTracker = CAMSHIFT_tracker("KLT", 100)
		else:
			print('{} is invalid face tracking model'.format(self.args.model))
			sys.exit(1)

		if 'SORT' in self.args.motmodel:
			self.facemulTracker = sorttracker()
		elif 'CENTROID' in self.args.motmodel:
			self.facemulTracker = centroidtracker()
		else:
			print('{} is invalid face mot model'.format(self.args.model))
			sys.exit(1)
	...
	def run_face_detector(self, frame):
		...
		faces = self.face_detector.predict(image)
	...
	async def detect_objects(self, q_in, q_out):
		...
		while self.running:
			...
			elements = [FrameInfo(t_frame, framenum),
						CameraImage(Image.fromarray(cv2.cvtColor(frame,cv2.COLOR_BGRA2RGB), mode='RGB'), (tempfaces, 1), self.image_t, self.args.dodraw),
	...
	async def detect_faces(self, q_in, q_out):
		skip_faces = 0
		face_limit = self.args.skipfaces
		prev_faces = None
		backSub = cv2.createBackgroundSubtractorMOG2()
		...
		while self.running:
			...
			runTrack = (skip_faces > 0 and prev_faces is not None)
			t0 = time()
			if runTrack:
				print("RUN TRACKING")
				skip_faces -= 1
				(faces, runTrack) = self.faceTracker.update_tracks(frame)
				t1 = time()
				print("TRACKER TIME " + str(t1-t0))
			if (not runTrack):
				print("RUN DETECTION")
				(faces, delta_t2) = await self.loop.run_in_executor(None, self.run_face_detector, frame)

				skip_faces = face_limit
				prev_faces = faces
				self.faceTracker.clear_tracks()
				self.facedb = []
				t1 = time()
				print("DETECTOR TIME " + str(t1-t0))
			...
			for face in faces:
				...
				if(w*h > max_area):
					max_area = w*h
				#self.dfdata.append([framenum, x, y, w, h])
				ofaces.append((x,y,w,h))
				if skip_faces == face_limit:
					self.faceTracker.add_tracker(frame, face, max_x, max_y)
			...
			output_tracks = self.facemulTracker.update(olist, conf)
			print(output_tracks)

			for track in output_tracks:
				framen, idnum, bb_left, bb_top, bb_width, bb_height, confidence, x1, y1, z1 = track
				self.dfdata.append([framenum, bb_left, bb_top, bb_width, bb_height, idnum])

			elements[1] = CameraImage(Image.fromarray(cv2.cvtColor(frame,cv2.COLOR_BGRA2RGB), mode='RGB'), (ofaces, max_area), self.image_t, self.args.dodraw)

			await q_out.put((frame, framenum, boxes, labels, scores, objd_skipped, elements, t_prev, output_tracks))
	...
	async def encode_features(self, q_in, q_out):
		with concurrent.futures.ThreadPoolExecutor() as pool:
			...
			while self.running:
				...
				(frame, framenum, boxes, labels, scores, objd_skipped, elements, t_prev, ofaces) = await q_in.get()
				...
				facedetections = [Detection((bb_left, bb_top, bb_width, bb_height), str(trackid), confidence, []) for (frame, trackid, bb_left, bb_top, bb_width, bb_height, confidence, x, y, z) in facesA0]
				...
				await q_out.put((framenum, detections, elements, time(), facedetections))

	async def track_objects(self, q_in, q_out):
		while self.running:
			(framenum, detections, elements, t_prev, facedetections) = await q_in.get()
			t1 = time()
	...
	async def process_results(self, q_in, q_out):
		while self.running:
			...
			for face in facedetections:
				fbox = face.to_tlbr()
				elements.append(DetectedFace(fbox, face.label, self.args.dodraw))
	...
	async def render_output(self, q_in):
		...
		try:
			while self.running:
				...
				t_now = time()
				t_e2e = t_now - t_frame
				self.e2ecount = self.e2ecount + t_e2e
	...
	async def start(self):
		...
		faceQueue = asyncio.Queue(maxsize=k)
		detectionQueue = asyncio.Queue(maxsize=k)
		resultQueue = asyncio.Queue(maxsize=k)
		drawQueue = asyncio.Queue(maxsize=k)
		self.queues = [cameraQueue, faceQueue, objectQueue, detectionQueue, resultQueue, drawQueue]
		...
		asyncio.ensure_future(self.detect_faces(faceQueue, objectQueue))
		asyncio.ensure_future(self.detect_objects(cameraQueue, faceQueue))
		...
		await render_task
		self.df = pd.DataFrame(self.dfdata, columns=self.header)
		self.df.to_csv(self.args.outputfaces, index=False)
		print(self.e2ecount)
		self.shutdown()
...
def get_arguments():
	...
	parser.add_argument('--facemodel', help='Face detection model to use.', default='Yunet')
	parser.add_argument('--trackermodel', help='Tracking model to use.', default='KCF')
	parser.add_argument('--motmodel', help='multi-object face tracking model to use.', default='SORT')
	parser.add_argument('--skipfaces', help='face detection/tracking interval', default=5, type=int)
	parser.add_argument('--outputfaces', help='output faces csv file', default='test.csv')
	parser.add_argument('--imgtmodel', help='Image transformation/blur model', default='BOX')
	parser.add_argument('--dodraw', help='Draw DP Blur instead of boxes?', default=False)
	parser.add_argument('--facescore', help='face confidence threshold', default=0.9, type=float)
